...
async function startHermes() {
  ...
  const startingConnection = (async () => {
    ...
    const spawnedHermesProcess = hermesProcess
    ...
    hermesProcess.once('error', error => {
      ...
      if (hermesProcess !== spawnedHermesProcess) {
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (hermesProcess !== spawnedHermesProcess) return
      hermesProcess = null
      connectionPromise = null
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => spawnedHermesProcess.exitCode === null && !spawnedHermesProcess.killed,
      rememberLog
    })
    ...
    commitConnectionFailure(connectionPromise, startingConnection, () => {
  ...
  connectionPromise = startingConnection
